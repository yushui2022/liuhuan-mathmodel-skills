...
def render_unit(task: dict, ph: dict, results: dict) -> str:
    ...
    if section == "附录":
        if unit_id == "APP-1":
            return header + "附录给出主要计算流程：读取附件→重采样→去趋势→FFT主峰→厚度计算→正弦/ Airy 拟合→输出JSON与图表。关键脚本为 step2_calc_results.py，一键入口为 skills/paper-workflow-orchestrator/scripts/run_all.py。\n"
